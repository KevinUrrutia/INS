...
%% load in the data
T = readtable("Epson_G370_20230130_174502.csv");
...
theta = zeros(1, round(reset_samples));
phi = zeros(1, round(reset_samples));

for ii = 1:round(1*fsamp)
    theta(ii) = atan(delta_v(1, ii) / sqrt((delta_v(2, ii)^2  + (delta_v(3, ii))^2)));
    phi(ii) = atan2(-delta_v(2, ii), -delta_v(3, ii));
end

ini_phi = mean(phi);
ini_theta = mean(theta);
